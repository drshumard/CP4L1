...
# Configuration
BACKEND_URL = "https://wellness-steps-2.preview.emergentagent.com/api"
...
class BackendTester:
    ...
    def test_login_endpoint(self):
        ...
        # First, we need to get the actual password that was generated by the webhook
        # Since we can't access the email, we'll test login with the password from signup
        # But first, let's test with wrong credentials
        ...
        # Test with correct credentials (using the password from signup)
        correct_payload = {
            "email": TEST_USER["email"],
            "password": TEST_USER["password"]
        }
        ...
        try:
            response = self.session.post(url, json=correct_payload)
